const { normalizeHttpResponse } = require("@kevboutin/azure-middy-util");
...
const securityHeadersMiddleware = (opts = {}) => {
    ...
    const securityHeadersMiddlewareAfter = async (request) => {
    ...
    const securityHeadersMiddlewareOnError = async (request) => {
